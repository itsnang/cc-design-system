 ...
 */
export async function validateFormStep(
  stepNumber: number,
  stepData: Partial<CompleteFormData>,
): Promise<{
  isValid: boolean;
  errors: Record<string, string>;
}> {
  try {
    const { personalInfoSchema, addressSchema, preferencesSchema } =
      await import("../../lib/validations/form-schemas");
    ...
    switch (stepNumber) {
      ...
      case 3:
        preferencesSchema.parse({
          ...
          notifications:
            stepData.notifications !== undefined
